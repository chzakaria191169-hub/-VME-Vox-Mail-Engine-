...
const ShiftEngine = require('./src/engine/ShiftEngine');
const SpamRescuer = require('./src/engine/SpamRescuer');
...
const state = {
    mailboxes: [],
    logs: [
        { timestamp: new Date().toISOString(), event: "SYSTEM_INIT", details: "Voxora Mail Engine (VME) initialized." }
    ],
...
server.listen(PORT, () => {
    console.log(`🚀 [VME SERVER & DASHBOARD] Live & Running on http://localhost:${PORT}`);
});
